...
CREATE TABLE IF NOT EXISTS maestro_job_template (           -- a job template maps to a step's subtype implementation
  job_type                  TEXT NOT NULL COLLATE "C",      -- job type is always same as the sub_type value
  version                   TEXT NOT NULL COLLATE "C",      -- a string to version job templates for a given job type
