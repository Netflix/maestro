...
public class SelMapStringArrayTest {
  ...
  public void setUp() throws Exception {
    ...
    map.put("foo", new String[] {"bar", "baz"});
    orig = SelMap.of(map);
  }
  ...
  public void testAssignOp() {
    ...
    assertEquals("MAP: {foo=[bar, baz]}", cur.type() + ": " + cur.toString());
  ...
  public void testUnbox() {
    ...
    assertEquals("[bar, baz]", Arrays.toString((String[]) map.get("foo")));
  }

  @Test
  public void testCallGet() {
    SelType res = orig.call("get", new SelType[] {SelString.of("foo")});
    assertEquals("STRING_ARRAY: [bar, baz]", res.type() + ": " + res.toString());
  ...
  public void testCallPut() {
    SelType res =
        orig.call(
            "put",
            new SelType[] {
              SelString.of("foo"), SelArray.of(new String[] {"baz", "bat"}, SelTypes.STRING_ARRAY)
            });
    assertEquals("STRING_ARRAY: [bar, baz]", res.type() + ": " + res.toString());
    res = orig.call("get", new SelType[] {SelString.of("foo")});
    assertEquals("STRING_ARRAY: [baz, bat]", res.type() + ": " + res.toString());
