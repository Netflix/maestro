...
public class SelMapLongArrayTest {
  ...
  public void setUp() throws Exception {
    ...
    map.put("foo", new long[] {123L, 111L});
    orig = SelMap.of(map);
  }
  ...
  public void testAssignOp() {
    ...
    assertEquals("MAP: {foo=[123, 111]}", cur.type() + ": " + cur.toString());
  ...
  public void testUnbox() {
    ...
    assertEquals("[123, 111]", Arrays.toString((long[]) map.get("foo")));
  }

  @Test
  public void testCallGet() {
    SelType res = orig.call("get", new SelType[] {SelString.of("foo")});
    assertEquals("LONG_ARRAY: [123, 111]", res.type() + ": " + res.toString());
  ...
  public void testCallPut() {
    SelType res =
        orig.call(
            "put",
            new SelType[] {
              SelString.of("foo"), SelArray.of(new long[] {222, 333}, SelTypes.LONG_ARRAY)
            });
    assertEquals("LONG_ARRAY: [123, 111]", res.type() + ": " + res.toString());
    res = orig.call("get", new SelType[] {SelString.of("foo")});
    assertEquals("LONG_ARRAY: [222, 333]", res.type() + ": " + res.toString());
